...
\end{lemma}
\begin{proof}
For the first representation, take the score matrix $S$ of a normalized embedding as in the proof of Theorem~\ref{thm:psd}. We already know that $S$ is positive semi-definite with equal diagonal entries. Condition (c) follows by unanimity of symmetric mean proximity rules (note that $S$ is symmetric, so $S_{ij} = S_{ji}$). Condition (d) follows by Lemma~\ref{lem:average-profile}. 
